...
Conglomerate::Conglomerate(vector<Connection *> con, bool bb){
    backbone_indestructable = bb;
    connections = move(con);
    updateConglomerate();
}

Conglomerate::Conglomerate(Polymer * pol, bool bb){
    backbone_indestructable = bb;
...
void Conglomerate::updateNeighboursUnbindingList(){
    ...
    for(int pol=0; pol<polymer_connections.size(); pol++){ //Loop polymers
        if(!(polymers[pol]->family==0 && backbone_indestructable)) {
...
vector<Conglomerate *> Conglomerate::checkSeparation(Connection * removed_connection){
    ...
    if(connections.empty()){
        ...
        output.push_back(new Conglomerate(removed_connection->polymers_in_connection[1], backbone_indestructable));
        return output;
    }
    ...
    if(separated){
        ...
        if(connected_polymers.size()==1){
            ...
            output.push_back(new Conglomerate(connected_polymers[0], backbone_indestructable));
            ...
            output.push_back(new Conglomerate(new_cong_conns, backbone_indestructable));
        }
    }
    return output;
}
